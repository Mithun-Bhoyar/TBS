...
else
{
	if(isset($_POST['update']))
	{
		...
		$errors = validate_update_hobby();
			
		if(empty($errors))
		{
			$checkhobbyduplication=mysqli_fetch_assoc(mysqli_query($conn,"SELECT id, hobbyname FROM hobbies WHERE hobbyname='".$_POST['hobbyname']."' and tokenid != '".base64_decode($_GET['tokenid'])."'"));



			if(!empty($checkhobbyduplication))
			{
	...
	$_POST = mysqli_fetch_assoc(mysqli_query($conn,"SELECT id, hobbyname, status FROM hobbies WHERE tokenid = '".base64_decode($_GET['tokenid'])."'"));
				...
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						<?php 
							...
							{ ?>
								<div class="alert alert-success"><?php echo "Hobby has been updated successfully";
								//sleep(3);
								echo "<script>setTimeout(\"location.href = 'http://localhost/TBS-PHP/SIR-PHP/task9-crud-hobbies/hobby_manage.php';\",1500);</script>";
							...
							{ ?>
								<div class="alert alert-danger"><?php echo "Unable to update hobby please try again"; ?></div>
							<?php } ?>
							
						<div class="panel-heading  panel-primary">
							<h4 class="panel-title">Update Hobby</h4>
						</div>
						<br/>
						<div class="col-md-12">
							<div class="form-group">
								<label class="control-label">Hobby</label><span class="text text-danger"> * <?php echo(isset($errors['hobbyname']))?$errors['hobbyname']:'';?></span>
